        ...
        <div className="max-w-4xl mx-auto min-h-[60vh] flex flex-col justify-center">
          ...
          >
            {skills.map((skill, index) => (
              <motion.div
                key={skill.name}
                className="group relative bg-card/80 backdrop-blur-sm flex-1 min-w-[180px] max-w-[220px] rounded-xl p-4 text-center border border-primary/20 shadow-lg transition-all duration-500 hover:shadow-xl hover:border-primary/40 hover:scale-105 hover:-translate-y-1"
                variants={itemVariants}
                whileHover={{ 
                  scale: 1.05, 
                  y: -5,
                ...
                <div className="relative z-10 space-y-3">
                  <div className="flex justify-center">
                    <Image
                      ...
                      width={32}
                      height={32}
                      className="w-8 h-8 group-hover:drop-shadow-lg transition-all duration-300 group-hover:rotate-12"
                      loading="lazy"
                    />
                  </div>
          ...
          >
            {skills.map((skill, index) => {
              ...
              const isOddInTwoColumn = skills.length % 2 === 1 && isLastItem;
              const isLastInThreeColumn = skills.length % 3 === 1 && isLastItem;
              
              return (
                <motion.div
                  key={skill.name}
                  className={`group relative bg-card/80 backdrop-blur-sm rounded-xl p-3 text-center border border-primary/20 shadow-lg transition-all duration-500 hover:shadow-xl hover:border-primary/40 hover:scale-105 hover:-translate-y-1 ${
                  ...
                  <div className="relative z-10 space-y-2">
                    <div className="flex justify-center">
                      <Image
                        ...
                        width={24}
                        height={24}
                        className="w-6 h-6 group-hover:drop-shadow-lg transition-all duration-300 group-hover:rotate-12"
                        loading="lazy"
                      />
                    </div>
